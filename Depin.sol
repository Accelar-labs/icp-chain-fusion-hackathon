 ...
 */
contract DPLDepin is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Funds the deployment status for a given token ID by the ICP oracle, passing the value in native tokens you want to fund.
     * @param _tokenId Token ID of the deployment
     */
    function fundDeployment(uint256 _tokenId) public payable {
        _fundingIds += 1;
        uint256 newFundingId = _fundingIds;

        Fundings[newFundingId] = Funding({
            id: newFundingId,
            tokenId: _tokenId,
            value: msg.value,
            executed: false
    ...
    /**
     * @dev Oracle updates funding deployment
     * @param _fundingId Funding ID of the deployment
     * @param _executed If the funding was executed or not
     */
    function updateFunding(uint256 _fundingId, bool _executed) public {
        require(icpOracle == msg.sender, "Only Oracle can change funding");
        Fundings[_fundingId].executed = _executed;
    }
